...
bounded `fractal_recall` only when read permission and exact source scope are proven;
label the result `read_only_recall`, and report that the lifecycle acknowledgment
is unavailable. If read permission or source scope is unproven, stop recall
and report that gap; lack of write authority does not establish read authority.
A synthetic observer,
