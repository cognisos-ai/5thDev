...
## Cursor source candidate adapter

The Cursor adapter registers `sessionStart`, `sessionEnd`, `preToolUse`,
`postToolUse`, `postToolUseFailure`, `subagentStart`, `subagentStop`,
`preCompact`, and `stop`. It uses Cursor's native lower-camel-case hooks, not
Claude's hook payloads. It projects only allowlisted event identity, opaque
session/turn/tool/agent identifiers, tool and subagent type, bounded outcome,
and compaction trigger. Prompts, tool inputs/outputs, errors, task descriptions,
summaries, file paths, workspace roots, transcript paths, model details, and
user email are never persisted. Observation is off by default and requires an
explicit absolute local log directory.

Cursor's documented cloud agents do not run `sessionStart` or `sessionEnd`, and
their early read-only turns may run without hooks. The adapter therefore does
not claim complete cloud-session coverage or completion enforcement. Cursor
does not expose a `postCompact` hook in the documented agent hook set, so this
adapter observes imminent compaction but not restoration. Source fixtures do
not prove that an installed Cursor version discovers or fires the package.
